...
generate_personal_data_html <- function(input_path, output_path) {
  ...
  exclude_keys <- personal_paths$exclude_keys %||% character()
  if (length(exclude_keys)) {
    df <- df %>% filter(!key %in% exclude_keys)
  }
...
generate_personal_data_html(
  resolve_configured_file(personal_paths, local_keys = c("csv"), gdrive_key = "csv_gdrive"),
